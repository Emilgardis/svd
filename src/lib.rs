...
trait ElementExt {
    fn get_child_text_try<K>(&self, k: K) -> Result<Option<String>>
        where String: PartialEq<K>, K: Debug + Clone;
    fn get_child_try<K>(&self, k: K) -> Result<&Element> where String: PartialEq<K>, K: Debug + Clone;
    fn debug(&self);
}
    ...
    fn parse(tree: &Element) -> Result<Peripheral> {
        ...
        Ok(Peripheral {
            ...
            interrupt:
            {
                ...
                interrupt
                // tree.get_child("interrupt").map_or(Ok(None), |inter| Interrupt::parse(inter).map(|i| Some(i)).chain_err(|| "Couldn't parse interrupt"))?,
            },
            registers: registers,
            derived_from: tree.attributes.get("derivedFrom")
                .map(|s| s.to_owned()),
            _extensible: (),
        })
    }
...
impl RegisterInfo {
    fn parse(tree: &Element) -> Result<RegisterInfo> {
        ...
        let mut fields_inter = tree.get_child("fields").map(|fs| fs.children.iter().map(Field::parse));
        let mut fields = Some(vec![]);
        if fields_inter.is_some() {
            for field_res in fields_inter.as_mut().unwrap() {
                fields.as_mut().unwrap().push(field_res.chain_err(|| format!("When trying to parse data of tag `fields` in register `{}`", name))?);
            }
        } else {
            fields = None;
        };
...
impl Field {
    fn parse(tree: &Element) -> Result<Field> {
        ...
        Ok(Field {
            ...
            access: tree.get_child("access").map_or(Ok(None), |access| Access::parse(access).map(|ac| Some(ac)).chain_err(|| "Couldn't parse access"))?,

            enumerated_values:
            {
                let enumerated_values_inter = tree.children
                .iter()
            ...
            write_constraint: tree.get_child("writeConstraint")
                .map_or(Ok(None), |constraint| WriteConstraint::parse(constraint).map(|c| Some(c)).chain_err(|| "Couldn't parse writeConstraint"))?,
...
impl BitRange {
    fn parse(tree: &Element) -> Result<BitRange> {
        let (end, start): (u32, u32) = if let Some(range) = tree.get_child("bitRange") {
            ...
            (bail_if_none!(bail_if_none!(parts.next(), "Invalid range").parse().ok(), "Failed to parse bitRange"), bail_if_none!(bail_if_none!(parts.next(), "Invalid range").parse().ok(), "Failed to parse bitRange"))
        } else if let (Some(lsb), Some(msb)) = (tree.get_child_text_try("lsb").ok().unwrap_or(None),
                                                                   tree.get_child_text_try("msb").ok().unwrap_or(None)) {
            (bail_if_none!(msb.parse().ok(), "Failed to parse msb"), bail_if_none!(lsb.parse::<u32>().ok(), "Failed to parse lsb"))
        } else {
            return Ok(BitRange {
                offset: bail_if_none!(tree.get_child_text_try("bitOffset")?.map_or(Ok(None), |offs| offs.parse().map(|int| Some(int)).chain_err(|| "Couldn't parse bitOffset"))?, "Couldn't get bitOffset"),
                width: bail_if_none!(tree.get_child_text_try("bitWidth")?.map_or(Ok(None), |offs| offs.parse().map(|int| Some(int)).chain_err(|| "Couldn't parse bitWidth"))?, "Couldn't get bitWidth"),
            });
        };
...
impl WriteConstraintRange {
    fn parse(tree: &Element) -> Result<WriteConstraintRange> {
        Ok(WriteConstraintRange {
            min: bail_if_none!(tree.get_child_text_try("minimum")?, "Couldn't get minimum").parse()?,
            max: bail_if_none!(tree.get_child_text_try("maximum")?, "Couldn't get maximum").parse()?,
        })
    }
}
            ...
            Ok(match field.as_ref() {
                "writeAsRead" => {
                    WriteConstraint::WriteAsRead(
                        parse::bool(tree.get_child_try(field.as_ref())?)?
                    )
                }
                "useEnumeratedValues" => {
                ...
                "range" => {
                    WriteConstraint::Range(
                        WriteConstraintRange::parse(tree.get_child_try(field.as_ref())?)?
                    )
                }
...
impl Defaults {
    fn parse(tree: &Element) -> Result<Defaults> {
        Ok(Defaults {
            size: tree.get_child("size").map_or(Ok(None), |t| parse::u32(t).map(|int| Some(int)).chain_err(|| "Couldn't parse size"))?,
            reset_value: tree.get_child("resetValue").map_or(Ok(None), |t| parse::u32(t).map(|int| Some(int)).chain_err(|| "Couldn't parse resetValue"))?,
            reset_mask: tree.get_child("resetMask").map_or(Ok(None), |t| parse::u32(t).map(|int| Some(int)).chain_err(|| "Couldn't parse resetMask"))?,
            access: tree.get_child("access").map_or(Ok(None), |val| Access::parse(val).map(|u| Some(u)).chain_err(|| "Couldn't parse access"))?,
            _extensible: (),
        })
    }
}
...
impl EnumeratedValues {
    fn parse(tree: &Element) -> Result<EnumeratedValues> {
        ...
        let values_inter = tree.children.iter()
        ...
        Ok(EnumeratedValues {
            ...
            usage: tree.get_child("usage").map_or(Ok(None),
                |usage| Usage::parse(usage).map(|u| Some(u)).chain_err(|| "Couldn't parse usage"))?,
...
impl EnumeratedValue {
    fn parse(tree: &Element) -> Result<Option<EnumeratedValue>> {
        ...
        Ok(Some(EnumeratedValue {
            name: bail_if_none!(tree.get_child_text_try("name")?, "Unable to get name"),
            description: tree.get_child_text_try("description")?,
            // Not the prettiest solution, but it works, probably can be hugely simplified
            value: tree.get_child("value").map_or(Ok(None),
                |elem| parse::u32(elem).map(|int| Some(int)).chain_err(|| "Couldn't parse value")
            )?,
            is_default: tree.get_child_text_try("isDefault")?.map_or(Ok(None), |t| t.parse().map(|t| Some(t)).chain_err(|| "Couldn't parse isDefault"))?,
            _extensible: (),
        }))
    }
}
