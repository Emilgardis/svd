...
pub fn u32(tree: &Element) -> Result<u32> {
    ...
    if text.starts_with("0x") || text.starts_with("0X") {
        ...
        u32::from_str_radix(&str::replace(&text["#".len()..], "x", "0"), 2).chain_err(|| "Couldn't parse binary")
    } else {
        text.parse().chain_err(|| format!("Failed to parse {:?}", text))
    }
}
...
pub fn dim_index(text: &str) -> Result<Vec<String>> {
    if text.contains('-') {
        ...
        let start = bail_if_none!(bail_if_none!(parts.next(), "Invalid dimIndex").parse::<u32>().ok(), "Couldn't parse dim start");
        let end = bail_if_none!(bail_if_none!(parts.next(), "Invalid dimIndex").parse::<u32>().ok(), "Couldn't parse dimIndex end") + 1;
